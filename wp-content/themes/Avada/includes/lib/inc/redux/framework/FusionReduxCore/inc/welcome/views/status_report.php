<?php
	...
	function fusionredux_get_support_object() {
		$obj = array();

	}

	function fusionredux_clean( $var ) {
		return sanitize_text_field( $var );
	}

	$sysinfo = FusionRedux_Helpers::compileSystemStatus( false, true );
...
?>
		...
		<?php
			...
			$posting['soap_client']['help'] = '<a href="#" class="fusionredux-hint-qtip" qtip-content="' . esc_attr__( 'Some webservices like shipping use SOAP to get information from remote servers, for example, live shipping quotes from FedEx require SOAP to be installed.', 'fusionredux-framework' ) . '">[?]</a>';

			if ( $sysinfo['soap_client'] == true ) {
				$posting['soap_client']['success'] = true;
			} else {
				$posting['soap_client']['success'] = false;
				$posting['soap_client']['note']    = sprintf( __( 'Your server does not have the <a href="%s">SOAP Client</a> class enabled - some gateway plugins which use SOAP may not work as expected.', 'fusionredux-framework' ), 'http://php.net/manual/en/class.soapclient.php' ) . '</mark>';
			}

			// DOMDocument
			$posting['dom_document']['name'] = 'DOMDocument';
			$posting['dom_document']['help'] = '<a href="#" class="fusionredux-hint-qtip" qtip-content="' . esc_attr__( 'HTML/Multipart emails use DOMDocument to generate inline CSS in templates.', 'fusionredux-framework' ) . '">[?]</a>';

			if ( $sysinfo['dom_document'] == true ) {
				$posting['dom_document']['success'] = true;
			} else {
				$posting['dom_document']['success'] = false;
				$posting['dom_document']['note']    = sprintf( __( 'Your server does not have the <a href="%s">DOMDocument</a> class enabled - HTML/Multipart emails, and also some extensions, will not work without DOMDocument.', 'fusionredux-framework' ), 'http://php.net/manual/en/class.domdocument.php' ) . '</mark>';
			}
			*/

			//// GZIP
			//$posting['gzip']['name'] = 'GZip';
			//$posting['gzip']['help'] = '<a href="#" class="fusionredux-hint-qtip" qtip-content="' . esc_attr__( 'GZip (gzopen) is used to open the GEOIP database from MaxMind.', 'fusionredux-framework' ) . '">[?]</a>';
			//
			//if ( $sysinfo['gzip'] == true ) {
			//    $posting['gzip']['success'] = true;
			//} else {
			//    $posting['gzip']['success'] = false;
			//    $posting['gzip']['note']    = sprintf( __( 'Your server does not support the <a href="%s">gzopen</a> function - this is required to use the GeoIP database from MaxMind. The API fallback will be used instead for geolocation.', 'fusionredux-framework' ), 'http://php.net/manual/en/zlib.installation.php' ) . '</mark>';
			//}
			...
			$posting = apply_filters( 'fusionredux_debug_posting', $posting );
...
<?php
	...
	<?php
		if ( ! empty( $sysinfo['fusionredux_instances'] ) && is_array( $sysinfo['fusionredux_instances'] ) ) {
			foreach ( $sysinfo['fusionredux_instances'] as $inst => $data ) {
...
<?php
									foreach ( $ext as $name => $arr ) {
										...
										echo '<a href="http://fusionreduxframework.com/extensions/' . str_replace( array(
												'_',
...
?>
	...
	<script type="text/javascript">
		jQuery( 'a.fusionredux-hint-qtip' ).on( 'click',
			function() {
				return false;
			}
		);
