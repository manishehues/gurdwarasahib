...
( function() {
    ...
    /**
		...
		renderLayoutBoxConditionsSection: function() {
			...
			this.item.find( '.awb-conditions-value' ).val( JSON.stringify( conditions ) );
		},
...
jQuery( document ).ready( function() {
	jQuery( '.awb-manage-conditions' ).click( function( e ) {
		var conditions = jQuery( this ).closest( '.pyre_metabox_field' ).find( '.awb-conditions-value' ).val(),
		optionsView;

		e.preventDefault();

		conditions  = '""' === conditions || 0 === conditions.length ? '{}' : conditions;
		optionsView = new FusionPageBuilder.layoutConditions( {
			'conditions': JSON.parse( conditions ),
			'item': jQuery( this ).closest( '.pyre_metabox_field' )
		} );

		jQuery( '.avada-dashboard' ).remove();
		jQuery( '#wpfooter' ).prepend( '<div class="avada-dashboard"></div>' ); // Needed for styles.
		jQuery( '#wpfooter .avada-dashboard' ).prepend( optionsView.render().el );
	} );
} );
