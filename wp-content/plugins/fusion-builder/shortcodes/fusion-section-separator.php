...
if ( fusion_is_element_enabled( 'fusion_section_separator' ) ) {
	...
	if ( ! class_exists( 'FusionSC_SectionSeparator' ) ) {
		...
		class FusionSC_SectionSeparator extends Fusion_Element {
			...
			/**
			 * An array of the hundred px separators.
			 *
			 * @access protected
			 * @since 3.2
			 * @var array
			 */
			protected $hundred_px_separators = [ 'slant', 'bigtriangle', 'curved', 'big-half-circle', 'clouds' ];
			...
			public function render( $args, $content = '' ) {
				...
				if ( 'triangle' === $divider_type ) {
					...
					$candy = '<svg class="fusion-horizon-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 ' . $y_min . ' 1024 178" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
					if ( 'top' === $divider_candy ) {
					...
					if ( 'top' === $divider_candy ) {
						$candy  = '<svg class="fusion-hills-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 74 1024 107" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
						...
						$candy  = '<svg class="fusion-hills-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 1 1024 107" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
					...
					$y_min = ( 'top' === $divider_candy ) ? '-0.5' : '0';
					$candy = '<svg class="fusion-hills-opacity-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 ' . $y_min . ' 1024 182" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
					if ( 'top' === $divider_candy ) {
					...
					$candy = '<svg class="fusion-waves-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 ' . $y_min . ' 1024 162" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
					...
					$candy = '<svg class="fusion-waves-opacity-candy" fill="' . $this->args['backgroundcolor'] . '" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" viewBox="0 ' . $y_min . ' 1024 216" preserveAspectRatio="none" ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg' ) . '>';
					...
					$this->args['svg_element'] .= '</svg>';
					$candy                      = '<div ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg-bg-image' ) . '></div>';
				} elseif ( 'waves_brush' === $divider_type ) {
					// Apply default height.
					...
					$candy = '<div ' . FusionBuilder::attributes( 'section-separator-shortcode-divider-svg-bg-image' ) . '></div>';
				} elseif ( 'custom' === $divider_type && '' !== $this->args['custom_svg'] ) {
			...
			public function svg_wrapper_attr() {
				...
				if ( ! isset( $this->args['additional_styles'] ) ) {
						...
						if ( 'boxed' === fusion_get_option( 'layout' ) ) {
							foreach ( $fusion_fwc_type['padding_flex'] as $size => $paddings ) {
								if ( ! empty( $fusion_fwc_type['padding_flex'][ $size ]['left'] ) || ! empty( $fusion_fwc_type['padding_flex'][ $size ]['right'] ) ) {
									$this->args['additional_styles'] .= 'large' === $size ? '' : '@media only screen and (max-width:' . $fusion_settings->get( 'visibility_' . $size ) . 'px) {';
									$this->args['additional_styles'] .= '.fusion-section-separator-' . $this->element_counter . ' .fusion-section-separator-svg {';
									$this->args['additional_styles'] .= 'position: relative;';
									...
									if ( ! empty( $fusion_fwc_type['padding_flex'][ $size ]['left'] ) && ! empty( $fusion_fwc_type['padding_flex'][ $size ]['right'] ) && false !== strpos( $fusion_fwc_type['padding_flex'][ $size ]['left'], '%' ) && false !== strpos( $fusion_fwc_type['padding_flex'][ $size ]['right'], '%' ) ) {
										...
										$this->args['additional_styles'] .= 'margin-left:calc(-' . ( (float) $fusion_fwc_type['padding_flex'][ $size ]['left'] / $scale ) . '% - ' . $padding . ') !important;';
										$this->args['additional_styles'] .= 'margin-right:calc(-' . ( (float) $fusion_fwc_type['padding_flex'][ $size ]['right'] / $scale ) . '% - ' . $padding . ') !important;';

										$this->args['additional_styles'] .= '}';
										$this->args['additional_styles'] .= '.fusion-section-separator-' . $this->element_counter . ' .fusion-section-separator-svg-wrapper{margin: 0 ' . $padding . ';}';
									} else {
										if ( ! empty( $fusion_fwc_type['padding_flex'][ $size ]['left'] ) ) {
											$this->args['additional_styles'] .= 'margin-left:-' . $fusion_fwc_type['padding_flex'][ $size ]['left'] . '!important;';
										}
										if ( ! empty( $fusion_fwc_type['padding_flex'][ $size ]['right'] ) ) {
											$this->args['additional_styles'] .= 'margin-right:-' . $fusion_fwc_type['padding_flex'][ $size ]['right'] . '!important;';
										}

										$this->args['additional_styles'] .= '}';
									}

									$this->args['additional_styles'] .= 'large' === $size ? '' : '}';
								}
							}
						} else {
				...
				$divider_height_arr  = [];
				$this->base_selector = '.fusion-section-separator.fusion-section-separator-' . $this->element_counter;
				foreach ( [ 'large', 'medium', 'small' ] as $responsive_size ) {
					$key = 'divider_height' . ( 'large' === $responsive_size ? '' : '_' . $responsive_size );
					...
					$divider_height_arr[ $key ] = $this->args[ $key ];
					$this->dynamic_css          = [];
					$media                      = 'large' === $responsive_size ? '' : sprintf( '@media only screen and (max-width:%spx)', $fusion_settings->get( 'visibility_' . $responsive_size ) );

					// Generate style rules.
					$selectors = [
						$this->base_selector . ' .fusion-section-separator-svg svg',
						$this->base_selector . ' .fusion-section-separator-svg-bg',
					];
					$this->add_css_property( $selectors, 'height', $this->args[ $key ] );
					$selectors = [
						$this->base_selector . ' .fusion-section-separator-spacer-height',
					];
					$this->add_css_property( $selectors, 'height', $this->args[ $key ] . ' !important' );
					$this->add_css_property( $selectors, 'padding-top', 'inherit !important' );

					if ( 'large' === $responsive_size ) {
						$this->args['additional_styles'] .= $this->parse_css();
					} else {
						$this->args['additional_styles'] .= sprintf( '%s { %s }', $media, $this->parse_css() );
					}
				}

				// Background Repeat.
				foreach ( [ 'large', 'medium', 'small' ] as $responsive_size ) {
					$key           = 'divider_repeat' . ( 'large' === $responsive_size ? '' : '_' . $responsive_size );
					$key_divider_h = 'divider_height' . ( 'large' === $responsive_size ? '' : '_' . $responsive_size );
					...
					if ( isset( $this->args['svg_element'] ) ) {
						...
						if ( 0 < strpos( $height, '%' ) ) {
							$value = sprintf( '%1$s%% 100%%', floatval( 100 / $this->args[ $key ] ) );
							$this->add_css_property( $selectors, 'background-size', $value );
						} else {
							$height = 0 < intval( $height ) ? $height : '100%';
							$value  = sprintf( '%1$s%% %2$s', floatval( 100 / $this->args[ $key ] ), $height );
							$this->add_css_property( $selectors, 'background-size', $value );
						}
			...
			public function spacer_attr() {
				...
				if ( ! empty( $fusion_fwc_type ) && isset( $fusion_col_type['type'] ) && $fusion_fwc_type['width_100_percent'] && '1_1' === $fusion_col_type['type'] ) {
					...
					if ( 'wide' === fusion_get_option( 'layout' ) ) {
						$attr['class'] .= ' fusion-section-separator-fullwidth';
					} else {
			...
			public function spacer_height_attr() {
				...
				if ( in_array( $this->args['divider_type'], $this->hundred_px_separators, true ) ) {
			...
			public function divider_svg_attr() {
				...
				if ( 'slant' === $this->args['divider_type'] && 'bottom' === $this->args['divider_candy'] ) {
					$attr['style'] = 'padding:0;margin-bottom:-3px;display:block;';
				}
			...
			public function divider_svg_bg_image_attr() {
				...
				if ( $this->args['svg_element'] ) {
					$attr['style'] = sprintf( 'background-image:url( data:image/svg+xml;base64,%s );', base64_encode( $this->args['svg_element'] ) );
	...
	fusion_builder_map(
		fusion_builder_frontend_data(
			...
			[
				...
				'help_url'   => 'https://theme-fusion.com/documentation/avada/elements/section-separator-element/',
				'params'     => [
					[
						'type'        => 'select',
