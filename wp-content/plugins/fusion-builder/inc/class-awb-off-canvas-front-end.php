...
class AWB_Off_Canvas_Front_End extends AWB_Layout_Conditions {
	 ...
	 */
	public static function render( $id, $force = false ) {
		...
		$html  = $style . '<div ' . self::wrap_attr( $id ) . '><div class="awb-off-canvas">' . $close_btn . '<div ' . self::attr( $id ) . '>' . $content . '</div></div></div>';
	...
	public static function wrap_attr( $id ) {
		...
		if ( ( 'sliding-bar' === $options['type'] && 'opened' === $options['off_canvas_state'] ) || ( function_exists( 'fusion_is_preview_frame' ) && fusion_is_preview_frame() ) ) {
			$atts['class'] .= ' awb-show';
		}
	...
	public static function attr( $id ) {
		...
		if ( 'sliding-bar' === $options['type'] && 'opened' === $options['off_canvas_state'] ) {
	...
	public static function close_button( $id ) {
		...
		$btn = '<span class="' . esc_attr( $class ) . '"></span>';
	...
	public static function get_style( $id ) {
		...
		if ( 'sliding-bar' === $options['type'] && 'opened' === $options['off_canvas_state'] && 'push' === $options['transition'] ) {
