...
(function( $ ) {
	...
	$.fusionredux.initEvents = function() {
		...
		$( '#toplevel_page_' + fusionredux.args.slug + ' .wp-submenu a, #wp-admin-bar-' + fusionredux.args.slug + ' a.ab-item' ).on( 'click',
			function( e ) {
	...
	$.fusionredux.tabCheck = function() {
		$( '.fusionredux-group-tab-link-a' ).on( 'click', function() {
	...
	$.fusionredux.tabControl = function() {
		...
		$( '.fusionredux-section-tabs ul li a' ).on( 'click',
			function() {
			...
			if ( id in current_tab_options ) {
				...
				if ( show === true ) {
                    ...
                    if ( 'info' === $( '#' + fusionredux.args.opt_name + '-' + id ).data( 'type' ) ) {
                        $( '#' + fusionredux.args.opt_name.replace( 'fusion_options', 'info' ) + '-' + id ).removeClass( 'hide' ).fadeIn( 300 );
                    } else {
                        $( '#' + fusionredux.args.opt_name + '-' + id ).parents( 'tr:first' ).removeClass( 'hide' ).fadeIn( 300 );
                            //$.fusionredux.initFields();
                    }
				} else if ( show === false ) {
                    if ( 'info' === $( '#' + fusionredux.args.opt_name + '-' + id ).data( 'type' ) ) {
                        $( '#' + fusionredux.args.opt_name.replace( 'fusion_options', 'info' ) + '-' + id ).addClass( 'hide' ).fadeOut( 300 );
                    } else {
                        $( '#' + fusionredux.args.opt_name + '-' + id ).parents( 'tr:first' ).addClass( 'hide' ).fadeOut( 300 );
                    }
				}
