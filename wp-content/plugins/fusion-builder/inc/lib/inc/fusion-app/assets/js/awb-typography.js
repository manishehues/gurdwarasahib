	...
	jQuery( document ).ready( function() {
		...
		AwbTypography.prototype.init = function() {
			...
			if ( ! this.isTypoSet ) {
				...
				jQuery( document ).on( 'awbTypoUpdate', this.dataChanged.bind( this ) );
			}
			...
			jQuery( document ).on( 'awbTypoNew awbTypoUpdate awbTypoDeleted', this.refreshAllStates.bind( this ) );
		...
		AwbTypography.prototype.dataChanged = function( event, data ) {
			var $input,
				slug;
			...
			$input = this.$el.find( '[data-subset="' + data.subset + '"]' );
			slug   = window.awbTypographySelect.stripValue( $input.val() );
			if ( slug === data.slug ) {
				$input.closest( '.input-wrapper' ).find( '.awb-global-label' ).attr( 'data-value', data.value ).html( data.value );
			}
		};
		...
		AwbTypography.prototype.setState = function( $input ) {
			...
			if ( 'string' === typeof value && '' !== value && value.includes( 'var(--' ) ) {
				...
				if ( 'undefined' !== typeof fontObject[ subset ] ) {
					...
					label.attr( 'aria-label', fontObject.label ).attr( 'data-value', value ).html( stringValue );
				} else {
					label.attr( 'aria-label', false ).attr( 'data-value', false ).html( fontObject.label );
				}
		...
		AwbTypography.prototype.renderBackupFontSelector = function( fontFamily ) {
			...
			if ( false === window.awbTypographySelect.isGoogleFont( fontFamily ) ) {
				$option.hide();
		...
		AwbTypography.prototype.destroy = function() {
			jQuery( document ).off( 'awbTypoUpdate', false, this.dataChanged );
		};
